...
void main()
{
    ...
    fprintf(plik, "x | f_szereg | f_scisle | liczba wyrazow\n");
    fprintf(stdout, "x | f_szereg | f_scisle | liczba wyrazow\n");
...
void writeOutToFile(double x, double sum, int n) {
    fprintf(plik, "%0.5f | %0.5f | %0.5f | %d\n", x, sum, atan(x), n);
    fprintf(stdout, "%0.5f | %0.5f | %0.5f | %d\n", x, sum, atan(x), n);
}
